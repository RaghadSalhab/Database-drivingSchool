...
public class Financial extends home implements Initializable {
   ...
   /* public void initialize(URL url, ResourceBundle resourceBundle) {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        Image newImage = new Image(getClass().getResource("report.png").toExternalForm());
    ...
    private String getStudentName(int enrollmentId) {
        ...
        try {
            ...
            String q = "Select fname,lname from driving_school.enrollment as e,driving_school.student as s where e.studentid=s.ssn and e.enrollmentid=" + enrollmentId;
            Statement st = DB.con.createStatement();
    ...
    private void updateChart() {
        try {
            double paymentSum = getSumOfAmounts("SELECT SUM(amount) FROM driving_school.payment WHERE EXTRACT(MONTH FROM paymentdate) = EXTRACT(MONTH FROM CURRENT_DATE)");
            double costSum = getSumOfAmounts("SELECT SUM(amount) FROM driving_school.cost WHERE EXTRACT(MONTH FROM costdate) = EXTRACT(MONTH FROM CURRENT_DATE)");
    ...
    void report(ActionEvent event) throws IOException
    {
        ...
        try {
            ...
            outputStream=new FileOutputStream(new File("FinReport.pdf"));
    ...
    void compRep(ActionEvent event) throws IOException
    {
        ...
        try {
            ...
            jr= JasperCompileManager.compileReport(jd);
            jp= JasperFillManager.fillReport(jr,null,DB.con);
            outputStream=new FileOutputStream(new File("CompeleteFinRep.pdf"));
            JasperExportManager.exportReportToPdfStream(jp,outputStream);
            outputStream.close();
            inputStream.close();
            // DBconn.con.close();
    ...
    void payandcost(ActionEvent event) throws IOException
    {
        ...
        try {
            ...
            jr= JasperCompileManager.compileReport(jd);
            jp= JasperFillManager.fillReport(jr,null,DB.con);
            outputStream=new FileOutputStream(new File("PayAndCost.pdf"));
            JasperExportManager.exportReportToPdfStream(jp,outputStream);
            outputStream.close();
            inputStream.close();
            // DBconn.con.close();
