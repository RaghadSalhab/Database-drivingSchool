...
public class enroll extends home  implements Initializable {
    ...
    void add(ActionEvent event) throws IOException {
        try {
            ...
            // Assuming lessonid is a SERIAL column
                ...
                try (PreparedStatement preparedStatement = DB.con.prepareStatement(addenroll)) {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        Image newImage = new Image(getClass().getResource("enrollment (3).png").toExternalForm());
        ...
        try {
            ...
            String query = "SELECT ssn FROM driving_school.student ";
    ...
    private void updateVehicleChoiceBox(Integer courseId) {
        try {
            ...
            try (PreparedStatement preparedStatement = DB.con.prepareStatement(query)) {
    ...
    void finish(ActionEvent event) throws IOException {
        ...
        try {
            PreparedStatement preparedStatement = DB.con.prepareStatement(updateLessonQuery);
        ...
        try {
            ...
            searchtxt.textProperty().addListener((observabl, oldvalue, newvalue) -> {
                filteredList.setPredicate(enrollmentTable -> {
                    ...
                    if (enrollmentTable.getStudentid().toLowerCase().indexOf(searchWord) > -1)
                        return true;
                    else if (enrollmentTable.getVehicleid().toLowerCase().indexOf(searchWord) > -1)
                        return true;
                        ...
                        return true;
                    else if (enrollmentTable.getVehicleid().toString().indexOf(searchWord) > -1)
                        return true;
                    else if (enrollmentTable.getStatus().toLowerCase().indexOf(searchWord) > -1)
                        return true;
                    else if (enrollmentTable.getInstructorid().toString().indexOf(searchWord) > -1)
                        return true;
                    else if (enrollmentTable.getStudentid().toString().indexOf(searchWord) > -1)
                        return true;
                    else
                        return false;
    ...
    void report(ActionEvent event) throws IOException
    {
        ...
        try {
            ...
            jp=JasperFillManager.fillReport(jr,null,DB.con);
