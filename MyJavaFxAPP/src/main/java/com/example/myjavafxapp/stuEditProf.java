        ...
        if (pas.getText() != null && ConfPas != null) {
            if (pas.getText().equals(ConfPas.getText())) {
                try {
                    ...
                    String updatePasswordQuery = "UPDATE driving_school.student SET password = ? WHERE ssn = ?";
                    java.sql.PreparedStatement updatePasswordStmt = DB.con.prepareStatement(updatePasswordQuery);
